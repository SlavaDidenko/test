import Block from './Block';

export class DemoForm extends Block {

  static get toolbox() {
    return {
      title: 'Demo form',
      icon: `<svg width="24" height="16" viewBox="0 0 24 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            <line x1="0.5" y1="0.5" x2="0.5" y2="15.5" stroke="black" stroke-linecap="round"/>
            <line x1="23.5" y1="0.5" x2="23.5" y2="15.5" stroke="black" stroke-linecap="round"/>
            <line x1="4.5" y1="4.5" x2="19.5" y2="4.5" stroke="black" stroke-linecap="round"/>
            <line x1="4.5" y1="11.5" x2="19.5" y2="11.5" stroke="black" stroke-linecap="round"/>
            <path d="M4.5 11.5V14M19.5 11.5V14" stroke="black"/>
            <path d="M4.5 4.5V2M19.5 4.5V2" stroke="black"/>
            </svg>`
    };
  }

  constructor({ data, api }){
    super();

    this.api = api;
    this.data = data;

    this.render = this.render.bind(this);
  }

  render() {
    const {
      title = '',
      description = '',
    } = this.data;

    let node = this.createNode();

    node.innerHTML = `
            <div data-block class="ce-page ce-tool ce-simpleBlock ce-demoForm">
                <header class="ce-tool-head ce-simpleBlock__head">
                    <h2 class="ce-tool-head__title"> Demo form </h2>
                </header>
                <div class="_paster_container ce-tool__item">
                      <div class="ce-tool__label">
                          Title <button class="_paster_btn parser-btn" type="button"></button>
                      </div>
                      <div data-field="title" contenteditable="true" class="_paster_target ce-tool__field ce-steps__item-text">${title}</div>
                  </div>
                  <div class="_paster_container ce-tool__item">
                      <div class="ce-tool__label">
                          Description <button class="_paster_btn parser-btn" type="button"></button>
                      </div>
                      <div data-field="description" contenteditable="true" class="_paster_target ce-tool__field ce-steps__item-text">${description}</div>
                  </div>
            </div>
        `;

    return this.nodes.wrapper;
  }
  save(node) {
    return {
      title: node.querySelector('[data-field="title"]').innerHTML,
      description: node.querySelector('[data-field="description"]').innerHTML,
    }
  }
}



export default DemoForm;